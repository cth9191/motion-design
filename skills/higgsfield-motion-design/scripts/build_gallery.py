...
def build():
    ...
    index += ['', '## Defaults and source differences', '',
        'New motion/launch adaptations default to music plus synchronized effects, no narration, and 15 seconds / 30fps unless explicit campaign choices or source-footage continuity require otherwise. Use one full-film request; references are optional. Retiming and replacing source silence are labeled adaptations. Most use 16:9; Exploded product starts at 9:16. Explicit campaign choices take precedence. Each full template defines its ending: Kinetic typography holds its final line; Glass UI freezes the complete final frame; Blueprint to building ends by un-plotting. Do not replace these with a house fade or assume a seamless loop.', '',
        ...
        f"[Source article]({catalog['source_url']}). Source mapping observed 2026-09-03. Version 4 archives the exact original prompts with verified hashes and keeps separate adaptation guides. Start from the original text, retaining its format and making targeted substitutions. Prefer a suitable current Seedance model; record a different source route as provenance. See [template-adaptation.md](template-adaptation.md).", '',
